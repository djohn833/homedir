...
beautiful.init("/usr/share/awesome/themes/dave/theme.lua")
...
if beautiful.wallpaper then
	show_wallpaper(beautiful.wallpaper)
end
-- }}}
...
tags = {
    names = { 'www', 'chat', 3, 4, 5, 6, 7, 8, 9 },
    layout = { layouts[4], layouts[4], layouts[1], layouts[1], layouts[1],
...
globalkeys = awful.util.table.join(
    ...
    awful.key({}, "XF86AudioLowerVolume",	function () os.execute('amixer -c 0 set Master 5%-') end),
    awful.key({}, "XF86AudioRaiseVolume",	function () os.execute('amixer -c 0 set Master 5%+') end),
    ...
    awful.key({ modkey }, "b",
        function ()
            awful.prompt.run({ prompt = 'BG cmd: ' },
                mypromptbox[mouse.screen].widget,
                function (s)
                    run_background(s, function (txt) naughty.notify({text = txt}) end)
                end)
        end),

    nil -- Just a dummy so I can always use commas after the other items
)
